...
public class PrintableReceiptTotals implements Printable{
	
	private SimpleDateFormat sdf = new SimpleDateFormat(UtilsCommon.getPropertyValue("receipt.title.dateFormat"));
	
	private LinkedHashMap<String, Double>  totMap;
	
	double titleY = 60;
	int spaceToList = 110;
	
	public PrintableReceiptTotals(LinkedHashMap<String, Double> lMap){
		totMap = lMap;
	}
	...
	public int print(Graphics g, PageFormat pageFormat, int page) {
		...
		int i = 0;
		for (Entry<String,Double> tot : totMap.entrySet()){
			g2d.drawString(tot.getKey(), (int) pageFormat.getImageableX(), spaceToList + (fm.getHeight() * i));
			if (tot.getValue() != null) {
				String formatted = String.format("€ %.2f", tot.getValue());
				int x = (int) (pageFormat.getImageableWidth() - fm.stringWidth(formatted));
				int y = spaceToList + (fm.getHeight() * i);
		...
		g2d.drawString("~~", (int) (pageFormat.getImageableWidth() / 2) - (fontMetrics.stringWidth("~~") / 2), spaceToList + (fm.getHeight() * totMap.size()) + 30);
